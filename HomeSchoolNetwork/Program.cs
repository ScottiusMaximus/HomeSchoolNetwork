...
namespace HomeSchoolNetwork {
	...
	class Program {
		...
		static void Main(string[] args) {
			...
			var d = new DirectoryInfo(downloads);
			var files = d.GetFiles("*.csv").OrderBy(x => x.CreationTime); 
			...
			var registrations = excelFile.Worksheet<Registration>(sheetName).ToList().OrderBy(x => x.UserId).ThenBy(x => x.ParentLastName).ThenBy(x => x.BirthDate).ToList();

			var filePath = $"{desktop}HSNMergeFormat.xlsx";
		...
		private static void DisplayInstructions() {
			Console.WriteLine("Ensure downloaded CSV file is in the downloads folder then press ENTER...");
			Console.WriteLine("Final Merge file will be in the Desktop HSNMergeFormat.xlsx");
			Console.ReadKey();
		}
